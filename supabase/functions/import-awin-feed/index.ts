...
function buildCreadUrl(awinMid, awinAffid, merchantUrl) {
...
serve(async (req)=>{
  ...
  const topCategoryDefault = config.top_category_default === "skincare" || config.top_category_default === "makeup" || config.top_category_default === "hair" ? config.top_category_default : null;
  ...
  const feedFormat = config.feed_format === "google_shopping" ? "google_shopping" : "awin";
  const feedUrlOverride = typeof config.feed_url === "string" && config.feed_url.trim().length > 0 ? config.feed_url.trim() : null;
  let feedUrl;
  if (feedUrlOverride) {
  ...
  if (feedUrl.startsWith("storage://")) {
    ...
    const { data: storageData, error: storageErr } = await supa.storage.from(bucket).download(objectPath);
  ...
  const previewBytes = Array.from(new Uint8Array(buf.slice(0, 32))).map((b)=>b.toString(16).padStart(2, "0")).join(" ");
  ...
  if (isGzipped) {
    ...
    try {
      ...
      const rawPreview = new TextDecoder("utf-8", {
        fatal: false
  ...
  if (feedFormat === "google_shopping") {
    idx = {
      ...
      aw_product_id: columns.indexOf("id"),
      search_price: columns.indexOf("sale_price"),
      store_price: columns.indexOf("price"),
      merchant_deep_link: columns.indexOf("aw_deep_link"),
  ...
  function parseInStock(raw, format) {
  ...
  const matchColumnIdx = config.match_column === "aw_product_id" ? idx.aw_product_id : idx.merchant_product_id;
  ...
  const categoryPathCounts = new Map();
  const updateActions = [];
  const linkActions = [];
  ...
  for(let i = 1; i < lines.length; i++){
    ...
    if (!matchValue) {
      countNoMatchId++;
      if (isOrdinary) ordinaryDiagnostic.excluded_no_match_id++;
      continue;
    }
    ...
    if (!isFinite(price) || price <= 0) {
      countNoPrice++;
      if (isOrdinary) ordinaryDiagnostic.excluded_no_price++;
      continue;
    }
    const inStock = parseInStock(fields[idx.in_stock] || "", feedFormat);
    if (!inStock) {
      countOOS++;
      if (isOrdinary) ordinaryDiagnostic.excluded_oos++;
      continue;
    }
    // URL — format-aware.
    ...
    if (feedFormat === "google_shopping") {
      ...
      wrappedUrl = rawMerchantUrl ? buildCreadUrl(config.awin_merchant_id, AWIN_PUBLISHER_ID, rawMerchantUrl) : "";
    }
    ...
    if (normEan && eanToProductId.has(normEan)) {
      ...
      matchedVia = "ean";
      countLinkViaEan++;
      if (isOrdinary) ordinaryDiagnostic.linked_via_ean++;
    }
    // Tier 2: MPN match
    if (!matchedProductId && normMpn && mpnToProductId.has(normMpn)) {
      ...
      matchedVia = "mpn";
      countLinkViaMpn++;
      if (isOrdinary) ordinaryDiagnostic.linked_via_mpn++;
    }
    ...
    if (!matchedProductId) {
      ...
      if (id) {
        ...
        if (isOrdinary) ordinaryDiagnostic.linked_via_name_exact++;
      }
    }
    ...
    if (!matchedProductId) {
      ...
      for (const candidateId of candidates){
        ...
        if (sourceSize === targetSize) {
          ...
          if (isOrdinary) ordinaryDiagnostic.linked_via_name_stripped++;
          break;
        }
    ...
    let finalTopCategory = cat.top_category;
    let finalTags = cat.tags;
  ...
  const categoryPathBreakdown = Array.from(categoryPathCounts.entries()).sort((a, b)=>b[1] - a[1]).slice(0, 100).map(([path, count])=>({
      path,
      count
    }));
