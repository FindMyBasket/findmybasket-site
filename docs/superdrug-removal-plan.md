...
> ── THE GENERATOR HAS NO SECOND USE. ADDED 23 AUGUST 2026, WORK-LIST ITEM 254. ──────────
>
> **This step says generate ON THE DAY. It does not say that the day is the ONLY day, and that
> is the more important fact.**
>
> `regen-gone-ids.mts` computes the drop set from products that have a live active-retailer row
> *right now* and no active-retailer row other than the departing one. **Once `retailers.active`
> is false, that query cannot be reconstructed.** The tool is not slow or stale afterwards — it
> is inoperable. Superdrug and Branded Beauty are both `active=false` today, so **neither
> departure's gone-set can ever be regenerated by this script.**
>
> **This step reads as advice about timing within a window. There is no window: there is one
> instant, and afterwards nothing.**
>
> It matters because **orphanhood keeps accruing after the flip**. 469 pages of these two
> retailers are in `products_active` and the sitemap today, in no gone-set — every one of them
> correctly excluded at generation time because a second retailer still carried it, and orphaned
> since when that row went out of stock. **No correct execution of step 6 could have caught them,
> because they were not orphans on the day.** Catching them needs a different query — orphans as
> of today — and that query does not exist. See work-list items 253 and 254.
