...
**[Corrected 15 August 2026] Every figure in this paragraph is an UPPER BOUND on the economics, because its denominator is a floor.** Commission per click is measured against our own outbound click log, and that log is written server-side but **triggered client-side**, by an onClick beacon the navigation does not depend on. Ad blockers, disabled JavaScript and right click open in new tab — which never fires the handler at all — are invisible to us and fully visible to AWIN.

> **This changes what the numbers mean rather than what they say.** Three point seven pence per click is the most flattering reading available. The real figure is lower, the real break even click count is higher, and **the real traffic multiple is larger than twenty four to forty times.**

**That matters precisely because the multiple is the binding constraint.** The commission rate question is settled and worth a factor of about four at the very most; the traffic gap is at least twenty four times and is now known to be understated. **Every correction found so far has moved the constraint in the same direction, and none of them has moved it in ours.**
