...
# FLUID OUNCES. Amazon states US fl oz where the catalogue states ml, so "1.7 fl oz" was
# read as DISAGREEING with "50ml" -- which it equals. That flag was wrong and the human
# acceptance it contradicted was right (item 105). 1 US fl oz = 29.5735 ml; the rounding
# below is to the nearest 5ml because pack sizes are, and an exact conversion would make
# 1.7 fl oz = 50.27ml disagree with a 50ml pack for the second time.
FLOZ = re.compile(r'(\d+(?:\.\d+)?)\s*fl\s*\.?\s*oz', re.I)

def floz_to_ml(s: str):
    out = set()
    for n in FLOZ.findall(s.lower()):
        ml = round(float(n) * 29.5735 / 5) * 5
        out.add(f'{ml}ml')
    return out
...
# BRITISH/AMERICAN SPELLING. Found by a human disagreeing with the machine, not by
# measuring: product 14684 "Revive Firming MoisturiSer 60ml" -- three live retailers --
# never scored at all against Amazon's "MoisturiZer", and the two candidates the matcher
# did offer were a 10ml and a one-retailer 60ml. The best row in the catalogue was
# invisible on a single letter. Work-list items 105 and 109 group D.
ISE_IZE = re.compile(r'(is|iz)(e|ed|es|ing|ation|ations|er|ers)\b', re.I)

def norm_spelling(s: str) -> str:
    return ISE_IZE.sub(lambda m: 'iz' + m.group(2), s)

def norm_tokens(s: str, brand: str = '') -> set:
    s = norm_spelling(strip_accents((s or '').lower()))
    s = GLUE.sub(' ', s)
...
# BOTH GAPS FIXED AND RE-MEASURED, 15 Aug 2026 (item 79: measure any change, including one
# made to fix a defect found while working):
#
#     committed (4d8f468)    top-1 116/155 (74.8%)   band n=51  precision 94.1%
#     with both gaps fixed   top-1 117/155 (75.5%)   band n=51  precision 94.1%
#
# A STRICT IMPROVEMENT: one row gained, nothing lost on the band anything is proposed from.
# Contrast the size tiebreak below, which gained two rows and cost 2.1 points of band
# precision and was therefore rejected.
#
# BUT NOTE HOW LITTLE THE CORPUS SAYS ABOUT IT, because this is the important part. Both
# gaps were found in the 49 UNMATCHED rows during the human pass, not in the 155
# EAN-confirmed pairs this is measured against. The corpus barely exercises them: it moves
...
# The justification is the two named cases, and the measurement's job here is only to prove
